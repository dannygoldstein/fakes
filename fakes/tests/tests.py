...
def test_fakes(science_image):
    ...
    rx = rng.uniform(size=100)
    ry = rng.uniform(size=100)

    # keep things bright
    mag = rng.uniform(low=15, high=18, size=100)
    ...
    outsky = SkyCoord(meascat['X_IMAGE'], meascat['Y_IMAGE'], unit='deg')
    i1, i2, sep, _ =  insky.search_around_sky(outsky, seplimit=0.5 * u.arcsec)

    # assert that all measured fake positions agree with the input positions
    # to within 0.5"
    assert len(i1) == 100
