...
`mag` and `coord` can be floating-point / coordinate arrays respectively. The program will read in the pixel information from the first fits extension of the passed image. The program does not modify the original HDU. Instead, it writes the fake-injected image to a new extension HDU. A fits binary table providing the truth table for the injected fakes is written to a second extension HDU, following the fake-injected image.
